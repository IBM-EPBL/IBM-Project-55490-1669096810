...
conn = ibm_db.connect(
    f"DATABASE={os.getenv('DB')};HOSTNAME={os.getenv('HOST')};PORT={os.getenv('PORT')};SECURITY=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID={os.getenv('USER')};PWD={os.getenv('PWD')}", '', '')
...
class ProductForm(Form):
    ...
    product_cost = StringField(
        'Product Cost', [validators.Length(min=1, max=200)])
    product_num = StringField(
        'Product QYT', [validators.Length(min=1, max=200)])
...
class LocationForm(Form):
    location_id = StringField(
        'Location ID', [validators.Length(min=1, max=200)])
...
def add_product_movements():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if from_location == to_location:
            ...
            if (alert_num <= 0):
                alert("Please update the quantity of the product {}, Atleast {} number of pieces must be added to finish the pending Product Movements!".format(
            ...
            if result != False:
                if (len(result)) > 0:
                    ...
                    if (alert_num <= 0):
                        alert(
                            "Please Add {} number of {} to {} warehouse!".format(-q, product_id, from_location))
            else:
                raise CustomError("There is no product named {} in {}.".format(
